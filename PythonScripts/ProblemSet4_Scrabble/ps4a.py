...
def isValidWord(word, hand, wordList):
    ...
    for letter in word:
        if letter in hand.keys():
            if x[letter] - word.count(letter) >= 0:
                continue
            else:
                return False
...
def playGame(wordList):
    ...
    """
    #initialize hand in case they try to replay b4 starting a new hand
    hand = {}
    #While user wants to play
    while True:
        #Ask the user for input as in step 1 above
        cmd = raw_input('Enter n to deal a new hand, r to replay the last hand, or e to end game:')
        
        #if a new game wants to be played
        if cmd == 'n':
